...
<details>
  <summary>Dictionary의 Key 값으로 설정할 수 있는 것들에 대해 설명해주세요.</summary>
  </br>
  <p>Mutable Type인 데이터 타입을 Key로 사용할 수 없습니다.  <br>
  파이썬에서 딕셔너리의 키로 사용될 수 없는 객체는 변경 가능한(mutable) 객체입니다. 변경 가능한 객체를 키로 사용할 수 없는 이유는, 딕셔너리의 키는 내부적으로 해시 함수를 사용하여 저장 및 검색되며, 이 해시 값이 일관성을 유지해야 합니다.<br>
  따라서, 리스트, 딕셔너리, 집합과 같은 변경 가능한 객체는 그 내용이 변경될 수 있으므로, 그것들을 딕셔너리의 키로 사용하면 예기치 않은 결과를 초래할 수 있습니다. <br>
  반면, 숫자, 문자열, 튜플(내부에 변경 불가능한 객체만 있을 경우)은 모두 해시 적용이 가능하므로 딕셔너리의 키로 사용할 수 있습니다.
</p>
