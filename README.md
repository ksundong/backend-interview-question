...
<details>
  <summary>가상 메모리의 페이징 매핑 과정에 대해 설명해보세요.</summary>
  </br>
  <p>가상 메모리의 동작방식의 핵심 기술은 크게 페이지, 페이지 테이블, 페이지 폴트, 스왑, TLB가 있습니다.</p>
    <ul>
      <li>페이지: 가상 메모리와 물리 메모리를 같은 크기의 블록 단위로 나눈 것을 말합니다.</li>
      <li>페이지 테이블: 가상 페이지에서 물리 페이지로 가는 주소의 매핑 정보를 저장합니다.</li>
      <li>페이지 폴트: 프로그램에 매핑되지 않은 가상 주소 접근시 페이지 폴트가 발생하는데 이때, 디스크에서 해당 페이지를 읽고 페이지 테이블을 업데이트합니다.</li>
      <li>스왑: 물리 메모리가 부족하면 사용하지 않는 페이지를 디스크에 저장하고, 필요할 때 다시 불러오는 방식을 의미합니다.</li>
      <li>TLB: 최근 매핑된 페이지를 저장하여, 페이지 테이블을 빠르게 접근하기 위한 캐시의 역할을 수행합니다</li>
    </ul>
  </br>
  <p>페이징 매핑 과정은 다음과 같습니다.</p>
  <ol>
    <li>CPU가 가상 주소를 TLB에서 확인한다.</li>
    <li>TLB에서 매핑 정보를 찾으면 즉시 물리 주소로 변환한다.</li>
    <li>TLB에서 찾지 못하면 페이지 테이블에 접근한다.</li>
    <li>페이지 테이블에서 매핑 정보 조회 후 변환한다.</li>
    <li>매핑 정보를 TLB에 저장한다.(캐싱)</li>
  </ol>
</details>
