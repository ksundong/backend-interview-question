...
### 네트워크
<details>
  <summary>고정 IP와 유동 IP의 차이를 말씀해주세요.</summary>
  </br>
  <p>고정 IP는 말 그대로 변하지 않고 컴퓨터에 고정적으로 부여된 IP입니다.</p>
  <p>한번 부여되면 IP 반납을 하기 전까지는 다른 장비에 부여할 수 없는 고유의 IP로 보안성이 우수하기 때문에 보안이 필요한 업체나 기관에서 사용합니다.</p>
  <p>유동 IP 역시 말그대로 변하는 IP입니다.</p>
  <p>인터넷 사용자 모두에게 고정 IP를 부여해 주기는 힘들기 때문에, 일정한 주기 또는 사용자들이 인터넷에 접속하는 매 순간마다 사용하고 있지 않은 IP 주소를 임시로 발급해 주는 IP이다.</p>
</details>
